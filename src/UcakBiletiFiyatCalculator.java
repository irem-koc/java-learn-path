...
public class UcakBiletiFiyatCalculator {
    public static void main(String[] args) {
        ...
        double firstly = mesafe / 10;
        double indirim_rate_yaş = 0;
        double indirim_rate_tip = 0;
        if (mesafe < 0 || age < 0 || (tip != 2 && tip != 1)) {
            System.out.println("Hatalı Veri Girdiniz !");
        } else {
            if (tip == 2) {
                indirim_rate_tip = 0.2;
            }
            if (age < 12) {
                indirim_rate_yaş = 0.5;
            } else if (age < 24) {
                indirim_rate_yaş = 0.1;
            } else if (age > 65) {
                indirim_rate_yaş = 0.3;

            }
            double indirim_yaş = indirim_rate_yaş * firstly;
            double indirim_tip = indirim_rate_tip * (firstly - indirim_yaş);
            double bilet = firstly - indirim_yaş - indirim_tip;
            if (tip == 2) {
                bilet *= 2;
            }
